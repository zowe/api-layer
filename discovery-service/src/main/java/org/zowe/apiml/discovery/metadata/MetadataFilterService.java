...
public class MetadataFilterService implements InitializingBean {
    ...
    private boolean verifyMetadataEntry(String key, String value, InstanceInfo info) {
        String[] metadataKeysToVerify = new String[]{
            "swaggerUrl",
            "graphqlUrl",
            "documentationUrl",
            "externalUrl"
        ...
        if (Arrays.stream(metadataKeysToVerify).anyMatch(metadataKey -> key.startsWith("apiml.") && key.endsWith(metadataKey))) {
    ...
    private boolean verifyCorsAllowedOrigins(String allowedOrigins, InstanceInfo info) {
        List<String> urls = Arrays.stream(allowedOrigins.split(",")).map(String::trim).collect(Collectors.toList());
    ...
    public void verifyAllowedDomains(InstanceInfo info) throws MetadataValidationException {
        ...
        if (info.getMetadata().containsKey("apiml.corsAllowedOrigins")) {
            boolean corsVerificationResult = verifyCorsAllowedOrigins(info.getMetadata().get("apiml.corsAllowedOrigins"), info);
        ...
        info.getMetadata().forEach((key, value) -> {
            boolean metadataVerificationResult = verifyMetadataEntry(key, value, info);
