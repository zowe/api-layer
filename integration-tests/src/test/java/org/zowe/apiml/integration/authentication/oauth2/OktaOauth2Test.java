...
public class OktaOauth2Test {
    ...
    void givenValidAccessToken_thenAllowAccessToResource() {
        ...
        Object accessToken = given().port(443).log().all().headers(headers).when().post("https://dev-95727686.okta.com:443/oauth2/default/v1/token?grant_type=client_credentials&scope=customScope")
        ...
        if (accessToken instanceof String) {
            ...
            given().log().all().headers("authorization", "Bearer " + token).get(dcUrl + "/discoverableclient/whoami").then().statusCode(200);
        } else {
