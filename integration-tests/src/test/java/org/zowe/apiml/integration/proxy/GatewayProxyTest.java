...
class GatewayProxyTest {
    ...
    static void init() throws Exception {
        ...
        conf = ConfigReader.environmentConfiguration().getCentralGatewayServiceConfiguration();
    }

    @Test
    void givenRequestHeader_thenRouteToProvidedHost() throws URISyntaxException {
        var scgUrl = String.format("%s://%s:%s/%s", conf.getScheme(), conf.getHost(), conf.getConnectPortForHost(conf.getHost()), "gateway/version");
        given()
    ...
    void givenBasePath_thenRouteToProvidedHost() throws URISyntaxException {
        String scgUrl1 = String.format("%s://%s:%s/%s", conf.getScheme(), conf.getHost(), conf.getConnectPortForHost(conf.getHost()), "apiml1/gateway/version");
        String scgUrl2 = String.format("%s://%s:%s/%s", conf.getScheme(), conf.getHost(), conf.getConnectPortForHost(conf.getHost()), "apiml2/gateway/version");
        given()
            .auth().preemptive().basic(CREDENTIALS.getUser(), CREDENTIALS.getPassword())
        .when()
            .get(new URI(scgUrl1))
    ...
    void givenRequestTimeoutIsReached_thenDropConnection() {
        String scgUrl = String.format("%s://%s:%s%s?%s=%d", conf.getScheme(), conf.getHost(), conf.getConnectPortForHost(conf.getHost()), DISCOVERABLE_GREET, "delayMs", DEFAULT_TIMEOUT + SECOND);
    ...
    class GivenClientCertificateInRequest {
        ...
        void givenRequestHeader_thenCertPassedToDomainGateway() {
            String scgUrl = String.format("%s://%s:%s%s", conf.getScheme(), conf.getHost(), conf.getConnectPortForHost(conf.getHost()), X509_ENDPOINT);
            given()
        ...
        void givenBasePath_thenCertPassedToDomainGateway() {
            String scgUrl = String.format("%s://%s:%s/%s%s", conf.getScheme(), conf.getHost(), conf.getConnectPortForHost(conf.getHost()), "apiml1", X509_ENDPOINT);
            given()
                .config(SslContext.clientCertValid)
            .when()
                .get(scgUrl)
    ...
    class GivenGatewayCertificatesRequest {
        ...
        void thenCertificatesChainProvided() throws URISyntaxException {
            String scgUrl = String.format("%s://%s:%s%s", conf.getScheme(), conf.getHost(), conf.getConnectPortForHost(conf.getHost()), CLOUD_GATEWAY_CERTIFICATES);
            String response =
                given()
                .when()
                    .get(new URI(scgUrl))
