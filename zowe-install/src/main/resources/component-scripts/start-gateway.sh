...
_BPX_JOBNAME=${ZOWE_PREFIX}${GATEWAY_CODE} java -Xms32m -Xmx256m -Xquickstart \
    ...
    -Dapiml.security.safEndpoint.enabled=${APIML_SECURITY_SAF_ENDPOINT_ENABLED:-false} \
    -Dapiml.security.safEndpoint.url=${APIML_SECURITY_SAF_ENDPOINT_URL:-http://localhost:${ZOWE_ZSS_SERVER_PORT}/saf-auth} \
    -Dapiml.security.saf.serviceResourceClass=${SERVICE_RESOURCE_CLASS:-ZOWE} \
    -Dapiml.security.saf.resourceNamePrefix=${RESOURCE_NAME_PREFIX:-APIML.} \
