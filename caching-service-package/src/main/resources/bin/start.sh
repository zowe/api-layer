...
_BPX_JOBNAME=${ZOWE_PREFIX}${CACHING_CODE} java -Xms16m -Xmx512m \
  ...
  -Dcaching.storage.infinispan.initialHosts="${ZWE_CACHING_SERVICE_INFINISPAN_INITIALHOSTS}" \
  -Dcaching.storage.infinispan.persistence.dataLocation="${ZWE_CACHING_SERVICE_INFINISPAN_PERSISTENCE_DATALOCATION}" \
  -Djgroups.bind.port=${ZWE_CACHING_SERVICE_INFINISPAN_JGROUPS_BIND_PORT:-7099} \
