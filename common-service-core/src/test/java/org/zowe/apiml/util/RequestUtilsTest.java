...
class RequestUtilsTest {
    ...
    BasicHeader contentLength = new BasicHeader("content-length", "10");
    ...
    void givenRequestWithoutHeaders_whenGetHeaders_thenGetEmptyList() {
        doReturn(new Header[0]).when(request).getHeaders();
        assertThat(wrapper.getHeaders().size(), is(0));
    }

    @Test
    void givenRequestWithHeaders_whenGetHeaders_thenGetListWithHeaders() {
        doReturn(new Header[] {contentLength, pragma}).when(request).getHeaders();
        assertThat(wrapper.getHeaders().size(), is(2));
        assertThat(wrapper.getHeaders(), hasItems(contentLength, pragma));
    }
    ...
    void givenRequestWithHeaders_whenSetExistingHeader_thenHeaderOverwritten() {
        ...
        assertThat(argument.getValue(), hasItemInArray(contentLength));
    ...
    void givenRequestWithCookieHeader_whenGetAllCookies_thenRetrievesAllCookies() {
        ...
        doReturn(new Header[] {contentLength, pragma, cookieMultiple, cookieApiml}).when(request).getHeaders();
    ...
    void givenRequestWithCookieHeader_whenSetCookie_thenCreatesOrOverwritesCookie() {
        doReturn(new Header[] {contentLength, pragma, cookieSingle}).when(request).getHeaders();
        ArgumentCaptor<Header[]> argument = ArgumentCaptor.forClass(Header[].class);
    ...
    void givenRequestWithCookieHeaderWithMultipleCookies_whenRemoveCookie_thenRemovesCookie() {
        doReturn(new Header[] {contentLength, pragma, cookieMultiple}).when(request).getHeaders();
        ArgumentCaptor<Header[]> argument = ArgumentCaptor.forClass(Header[].class);
