...
_BPX_JOBNAME=${ZWE_zowe_job_prefix}${APIML_CODE} ${JAVA_BIN_DIR}java \
    ...
    -Dotel.exporter.otlp.endpoint="${ZWE_configs_telemetry_exporter_endpoint:-localhost:4318}" \
    -Dotel.resource.attributes.deployment.environment="${ZWE_configs_telemetry_attributes_deployment_environment:-prod}" \
    -Dotel.resource.attributes.mainframe.lpar.name=${ZWE_configs_telemetry_attributes:-ZWE_haInstance_id} \
    -Dotel.resource.attributes.os.type="${:-zos}" \
    -Dotel.resource.attributes.service.instance.id="${:-}" \
    -Dotel.resource.attributes.service.name="${:-}" \
    -Dotel.resource.attributes.service.namespace="${:-}" \
    -Dotel.resource.attributes.service.version="${:-}" \
    -Dotel.resource.attributes.zos.smf.id="${}" \
    -Dotel.resource.attributes.zos.sysplex.name="${ZWE_configs_telemetry_attributes_deployment_sysplex_name:-}" \
