...
_BPX_JOBNAME=${ZOWE_PREFIX}${GATEWAY_CODE} java \
    ...
    -Dapiml.security.saf.urls.authenticate=${APIML_SECURITY_SAF_URLS_AUTHENTICATE:-"https://${EXPLORER_HOST}:${GATEWAY_SERVICE_PORT}/zss/api/v1/saf/authenticate"} \
    -Dapiml.security.saf.urls.verify=${APIML_SECURITY_SAF_URLS_VERIFY:-"https://${EXPLORER_HOST}:${GATEWAY_SERVICE_PORT}/zss/api/v1/saf/verify"} \
