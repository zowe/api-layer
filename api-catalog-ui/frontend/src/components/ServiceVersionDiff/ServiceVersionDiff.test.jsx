...
describe('>>> ServiceVersionDiff component tests', () => {
    it('Should display service version diff with no data', () => {
        ...
        expect(serviceVersionDiff.find('Button').first().exists()).toEqual(true);
        expect(serviceVersionDiff.find('Button').first().prop('children')).toEqual('Go');
    });
    ...
    it('Should call getDiff when button pressed', () => {
        ...
        const serviceVersionDiff = shallow(
            <ServiceVersionDiff getDiff={getDiff} serviceId="service" versions={["v1", "v2"]} version1="v1" version2="v2"/>);
        
        serviceVersionDiff.find('Button').first().simulate('click');
