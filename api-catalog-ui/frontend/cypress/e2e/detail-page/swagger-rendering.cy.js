    ...
    [
        ...
        it(`Rendering Swagger for ${service.serviceName}`, () => {
            ...
            cy.get('div.tabs-swagger > div.serviceTab > div.header > a:nth-child(2)')
                .as('serviceHomepage')
                .should('contain', 'Service Homepage');

            cy.get('@serviceHomepage').should('have.attr', 'title', 'Open Service Homepage');

            cy.get('div.tabs-swagger').find('.apiInfo-item').get('h6:nth-Child(1)').as('basePath');

            cy.get('@basePath').get('label').should('contain', 'API Base Path:');
            ...
            cy.get('div.tabs-swagger').find('.apiInfo-item').get('h6:nth-Child(2)').as('sId');

            cy.get('@sId').get('label').should('contain', 'Service ID:');

            cy.get('@sId').get('#serviceId').should('contain', service.serviceId);

            cy.get('#swagger-label').should('exist').should('contain', 'Swagger');
            ...
            cy.get('@swaggerContainer').get('div.information-container > section div.info .main').as('mainInfo');
            ...
            cy.get('@instanceUrlLabel').should('exist').should('contain', 'Instance URL:');
            ...
            cy.get('@operationBlock').find('.operation-tag-content').should('exist');

            cy.get('@operationBlock').get('.operation-tag-content').find('.opblock').should('exist');

            cy.get('@operationBlock')
                .get('.operation-tag-content > .opblock > .opblock-summary > button:first-child')
                .should('exist');
            cy.get('@operationBlock').get('button:first-child').should('have.class', 'opblock-summary-control');
