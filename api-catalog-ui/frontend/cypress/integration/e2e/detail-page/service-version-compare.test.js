...
const PATH_TO_VERSION_SELECTORS =
    '.detail-page > div.content-description-container > div > div.serviceTab > div.api-diff-container > div.api-diff-form > div:nth-child(2) > div > div';
const PATH_TO_VERSION_SELECTORS2 =
    '.detail-page > div.content-description-container > div > div.serviceTab > div.api-diff-container > div.api-diff-form > div:nth-child(4) > div > div';
...
describe('>>> Service version compare Test', () => {
    ...
    it('Should switch to compare tab when clicked', () => {
        ...
        cy.get(
            '#root > div > div.content > div.detail-page > div.content-description-container > div > div.serviceTab > div.tabs-container > span:nth-child(4) > p'
        ).should('contain.text', 'Compare');
        cy.get('.nav-tab:nth-child(4)').click();
        cy.get('.api-diff-container').should('exist');

        cy.get('.api-diff-form').should('exist');
        cy.get('.api-diff-form > div:nth-child(2) > label').eq(0).should('exist');
        cy.get('.api-diff-form > p:nth-child(1)').eq(0).should('contain.text', 'Compare');
        cy.get('.api-diff-container > div.api-diff-form > p:nth-child(3)').eq(0).should('exist');
        cy.get('.api-diff-container > div.api-diff-form > p:nth-child(3)').eq(0).should('contain.text', 'with');
        cy.get('.api-diff-form > div:nth-child(2) > label').eq(0).should('exist');
        cy.get('.api-diff-container > div.api-diff-form > div:nth-child(2) > label')
            .eq(0)
            .should('contain.text', 'Version');
        cy.get('.api-diff-container > div.api-diff-form > div:nth-child(4) > label').eq(0).should('exist');
