...
import ssl
import yaml
from src.custom_http_client import HttpResponse, HttpClient
...
def mock_config_loader():
    ...
    with patch("src.custom_http_client.ConfigLoader.__init__", mock_init), \
        patch("src.custom_http_client.config_loader.config", mocked_yaml):
        yield
...
def mock_ssl_context():
    ...
    with patch("src.custom_http_client.ssl.create_default_context", return_value=mock_ssl):
        yield mock_ssl


@pytest.fixture
def mock_aiohttp_session():
    with patch("src.custom_http_client.aiohttp.ClientSession") as mock_session_cls:
