...
class ZoweTest {
    ...
    @Test
    void givenZoweInstance_whenGetAuthenticationScheme_thenReturnProperType() {
        assertEquals(AuthenticationScheme.ZOWE_JWT, new Zowe().getAuthenticationScheme());
    }
    ...
    void givenRouteDefinition_whenApply_thenFulfillFilterFactorArgs() {
        ...
        new Zowe().apply(serviceInstance, routeDefinition, authentication);

        assertEquals(1, routeDefinition.getFilters().size());
        FilterDefinition filterDefinition = routeDefinition.getFilters().get(0);
        assertEquals("ZoweFilterFactory", filterDefinition.getName());
    }
}
