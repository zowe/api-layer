 ...
 */
package com.broadcom.apiml.library.security.login;

import com.broadcom.apiml.library.security.config.SecurityConfigurationProperties;
import com.broadcom.apiml.library.security.token.TokenAuthentication;
...
public class ZosmfAuthenticationProvider implements AuthenticationProvider {
    ...
    private String getURI(String zosmf) {
        String uri = "https://ca32.ca.com:4434";
        if (uri == null) {
    ...
    private String readDomain(String content) {
        ...
        try {
            ...
            log.error("Error parsing z/OSMF message.");
        ...
        if (zosmfNode != null && zosmfNode.has(ZOSMF_DOMAIN)) {
            ...
            log.error("z/OSMF message does not contain field '{}'.", ZOSMF_DOMAIN);
            throw new AuthenticationServiceException("z/OSMF domain cannot be read.");
        }
    }
