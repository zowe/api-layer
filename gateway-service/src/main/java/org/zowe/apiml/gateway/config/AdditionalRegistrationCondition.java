...
public class AdditionalRegistrationCondition implements Condition {
    ...
    public boolean matches(ConditionContext context, AnnotatedTypeMetadata metadata) {
        ...
        List<String> additionalKeys = ((StandardServletEnvironment) context.getEnvironment()).getSystemEnvironment()
            .entrySet().stream().map(e -> e.getKey().toUpperCase()).filter(key -> key.startsWith(AdditionalRegistrationConfig.PREFIX))
            .collect(Collectors.toList());
        boolean isAdditionalTriggered = dcUrls != null || !additionalKeys.isEmpty();
        log.debug("isAdditionalTriggered: " + isAdditionalTriggered);
        return isAdditionalTriggered;
    }
}
