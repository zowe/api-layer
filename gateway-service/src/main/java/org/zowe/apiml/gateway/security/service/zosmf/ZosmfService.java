...
public class ZosmfService extends AbstractZosmfService {
    ...
    public ZaasResponse exchangeAuthenticationForZosmfToken(String token, AuthSource.Parsed authSource) throws ServiceNotFoundException {
        switch (authSource.getOrigin()) {
            case ZOSMF:
                return new ZaasResponse(ZosmfService.TokenType.JWT.getCookieName(), token);
            case ZOWE:
                String ltpaToken = authenticationService.getLtpaToken(token);
                if (ltpaToken != null) {
                    return new ZaasResponse(ZosmfService.TokenType.LTPA.getCookieName(), ltpaToken);
                }
            default:
                ...
                if (zosmfTokens.containsKey(JWT)) {
                    return new ZaasResponse(ZosmfService.TokenType.JWT.getCookieName(), zosmfTokens.get(JWT));
                } else if (zosmfTokens.containsKey(LTPA)) {
                    return new ZaasResponse(ZosmfService.TokenType.LTPA.getCookieName(), zosmfTokens.get(LTPA));
                }
        }
