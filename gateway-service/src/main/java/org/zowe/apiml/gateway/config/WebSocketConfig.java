...
public class WebSocketConfig {
    ...
    WebSocketClient webSocketClient(HttpConfig config) {
        ...
        var client = new ApimlWebSocketClient(config.secureSslContext(), webSocketContainer); // using netty client (async) with Tomcat backend (sync) can lead to conditions where frames are written
        return client;
    }
