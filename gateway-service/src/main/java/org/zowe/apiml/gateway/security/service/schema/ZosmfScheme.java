...
public class ZosmfScheme implements AbstractAuthenticationScheme {
    ...
    public AuthenticationCommand createCommand(Authentication authentication, AuthSource authSource) {
        final AuthSource.Parsed queryResponse = authSourceService.parse(authSource);
    ...
    public class ZosmfCommand extends AuthenticationCommand {
        ...
        public void apply(InstanceInfo instanceInfo) {
            ...
            Optional<AuthSource> jwtToken = authSourceService.getAuthSource();
            jwtToken.ifPresent(token -> {
                // parse JWT token to detect the source (z/OSMF / Zowe)
                AuthSource.Parsed queryResponse = authSourceService.parse(token);
                switch (queryResponse.getSource()) {
                    case ZOSMF:
                        ...
                        setCookie(context, ZosmfService.TokenType.JWT.getCookieName(), (String)token.getSource());
                        break;
                    case ZOWE:
                        // user use Zowe own JWT token, for communication with z/OSMF there should be LTPA token, use it
                        final String ltpaToken = authSourceService.getLtpaToken(token);
            ...
            jwtToken.ifPresent(token -> {
                ...
                switch (queryResponse.getSource()) {
                    case ZOSMF:
                        ...
                        createCookie(cookies, ZosmfService.TokenType.JWT.getCookieName(), (String)token.getSource());
                        break;
                    case ZOWE:
                        final String ltpaToken = authSourceService.getLtpaToken(token);
