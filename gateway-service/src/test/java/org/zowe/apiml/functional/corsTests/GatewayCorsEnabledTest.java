...
class GatewayCorsEnabledTest extends FunctionalTestWithTwoServices {
    ...
    class GivenCorsIsAllowed {
        ...
        void whenSimpleRequestArrives_thenCorsHeadersAreSetAndOnlyTheOnesByGateway() throws Exception {
        ...
        void whenTheServiceIsSet_thenCorsHeadersAreSetAndOnlyTheOnesByGateway() throws Exception {
            mockValid200HttpResponseWithAddedCors();
            applicationRegistry.setCurrentApplication(serviceWithCustomConfiguration.getId());
            discoveryClient.createRefreshCacheEvent();
            // Preflight request
        ...
        @Test
        void whenRequestWithOriginComes_thenOriginIsntPassedToSouthbound() throws Exception {
            // There is request to the southbound server and the CORS headers are properly set on the response
            mockValid200HttpResponseWithAddedCors();
            applicationRegistry.setCurrentApplication(serviceWithCustomConfiguration.getId());
            discoveryClient.createRefreshCacheEvent();
