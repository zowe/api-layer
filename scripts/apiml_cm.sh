...
function usage {
    ...
    echo "  apiml_cm.sh --action setup --local-ca-alias ... --local-ca-filename ... --local-ca-dname ... --local-ca-pass ... --local-ca-validity ..."
}
...
LOCAL_CA_PASS="local_ca_password"
...
function create_certificate_authority {
    ...
    keytool -genkeypair -v -alias ${LOCAL_CA_ALIAS} -keyalg RSA -keysize 2048 -keystore ${LOCAL_CA_FILENAME}.keystore.p12 \
        -dname "${LOCAL_CA_DNAME}" -keypass ${LOCAL_CA_PASS} -storepass ${LOCAL_CA_PASS} -storetype PKCS12 -validity ${LOCAL_CA_VALIDITY} \
    ...
    keytool -export -v -alias ${LOCAL_CA_ALIAS} -file ${LOCAL_CA_FILENAME}.cer -keystore ${LOCAL_CA_FILENAME}.keystore.p12 -rfc \
        -keypass ${LOCAL_CA_PASS} -storepass ${LOCAL_CA_PASS} -storetype PKCS12
}

function create_service_certificate_and_csr {
    echo "Generate service private key and service:"
    keytool -genkeypair -v -alias ${SERVER_ALIAS} -keyalg RSA -keysize 2048 -keystore ${SERVER_FILENAME}.keystore.p12 -keypass ${SERVER_PASS} -storepass ${SERVER_PASS} \
        -storetype PKCS12 -dname "${SERVER_DNAME}" -validity ${SERVER_VALIDITY}

    echo "Generate CSR for the the service certificate:"
    keytool -certreq -v -alias ${SERVER_ALIAS} -keystore ${SERVER_FILENAME}.keystore.p12 -storepass ${SERVER_PASS} -file ${SERVER_FILENAME}.csr \
...
function sign_csr_using_local_ca {
    ...
    keytool -gencert -v -infile ${SERVER_FILENAME}.csr -outfile ${SERVER_FILENAME}_signed.cer -keystore ${LOCAL_CA_FILENAME}.keystore.p12 \
        -alias ${LOCAL_CA_ALIAS} -keypass ${LOCAL_CA_PASS} -storepass ${LOCAL_CA_PASS} -storetype PKCS12 \
...
function import_signed_certificate_and_ca_certificate {
    ...
    keytool -importcert -v -trustcacerts -noprompt -file ${LOCAL_CA_FILENAME}.cer -alias ${LOCAL_CA_ALIAS} -keystore ${SERVER_FILENAME}.truststore.p12 -storepass ${SERVER_PASS} -storetype PKCS12

    echo "Import the Certificate Authority to the keystore:"
    keytool -importcert -v -trustcacerts -noprompt -file ${LOCAL_CA_FILENAME}.cer -alias ${LOCAL_CA_ALIAS} -keystore ${SERVER_FILENAME}.keystore.p12 -storepass ${SERVER_PASS} -storetype PKCS12

    echo "Import the signed CSR to the keystore:"
    keytool -importcert -v -trustcacerts -noprompt -file ${SERVER_FILENAME}_signed.cer -alias ${SERVER_ALIAS} -keystore ${SERVER_FILENAME}.keystore.p12 -storepass ${SERVER_PASS} -storetype PKCS12
}
